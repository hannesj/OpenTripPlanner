...
- Ask which remote a feature branch should be pushed to for creating PRs to upstream
...
3. Ask which remote to push the feature branch to (e.g., `git push -u <remote> my-feature`)
4. Create PR in upstream repository using `gh pr create --repo opentripplanner/OpenTripPlanner --head <remote>:my-feature --base dev-2.x`
   - Replace `<remote>` with the appropriate remote name (e.g., `entur`, `origin`, or your fork name)
