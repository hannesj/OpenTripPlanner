...
public class NetworkLinkerLibrary {
    ...
    public LinkRequest connectVertexToStreets(PoiVertex v) {
        LinkRequest request = new LinkRequest(this);
        request.connectVertexToStreets(v, new TraverseModeSet(TraverseMode.WALK,
                TraverseMode.BICYCLE),new LinkRequest.StreetLinkFactory<PoiVertex>() {
            @Override
            public Collection<? extends Edge> connect(StreetVertex sv, PoiVertex v) {
                return Arrays.asList(new PoiLinkEdge(sv, v), new PoiLinkEdge(v, sv));
            }
        });
        return request;
    }
